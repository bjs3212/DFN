# 유광남 박사님의 github에 있는 SMMO (Scattering Matrix Method for multilayer Optics) 긁어옴
# https://github.com/phykn/smmo/tree/main 참고

# Ref : 
# https://journals.aps.org/prb/pdf/10.1103/PhysRevB.38.9945
# https://opg.optica.org/view_article.cfm?pdfKey=cdfb4cd3-9eab-410d-8fe32114267745b3_3255
...
class SMMO:
    ...
    def get_smatrix_components(
        ...
        for i, layer in enumerate(layers):
            ...
            if layer['coherence'] == False : phase[i] = abs(np.exp(1j * self.get_kz(n[i], cos_q[i], self.ws) * d[i]))
            else : phase[i] = np.exp(1j * self.get_kz(n[i], cos_q[i], self.ws) * d[i])
